...
class SavedAddress extends React.Component {
  ...
  render() {
    ...
    if (currentAddressId == this.props.address.id) {
      return (
        <div class="shadow-lg rounded-lg flex flex-col my-4 px-8 py-4 w-3/4 mx-auto border-b-2 border-purple group cursor-pointer" onClick={() => this.props.updateAddress()}>
          <div class="flex justify-between w-full mb-3 items-center">
            <h1 class="capitalize text-grey-darker text-xl font-heavy">{address.name}</h1>
            {address.primary && <button class="rounded-full text-purple border-solid border border-purple py-1 px-4 text-xs">primary</button>}
          </div>
